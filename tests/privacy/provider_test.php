...
final class provider_test extends provider_testcase {
     ...
     * #[CoversClass(paygw_airtelafrica\privacy\provider)]
     */
    public function setUp(): void {
        ...
        $saved = helper::save_payment($id, 'enrol_fee', 'fee', $feeid, $user->id, 30, 'EUR', 'airtelafrica');
        ...
        $paygen->create_payment(
            ['accountid' => $accountid, 'amount' => 2, 'gateway' => 'airtelafrica', 'userid' => $user->id]
        );
        $this->user = $generator->create_user();
        $id = $paygen->create_payment(
            ['accountid' => $accountid, 'amount' => 1, 'gateway' => 'airtelafrica', 'userid' => $this->user->id]
        );
        $paygen->create_payment(
    ...
    public function test_get_metadata(): void {
        ...
        $this->assertEquals('paygw_airtelafrica', $metadata->get_component());
        ...
        $this->assertEquals('paygw_airtelafrica', $table->get_name());
        $this->assertEquals('privacy:metadata:paygw_airtelafrica', $table->get_summary());
    ...
    public function test_provider(): void {
        global $DB;
        $this->assertEquals(2, $DB->count_records('paygw_airtelafrica', []));
        ...
        $this->assertEquals('paygw_airtelafrica', $contextlist->get_component());
        ...
        $this->assertEquals(4, $DB->count_records('paygw_airtelafrica', []));
    }
    ...
    private function create_user_with_payment(): stdClass {
        ...
        $id = $generator->get_plugin_generator('core_payment')->create_payment(
            ['accountid' => $account->get('id'), 'amount' => 2, 'gateway' => 'airtelafrica', 'userid' => $user2->id]
        );
        $data = new stdClass();
        $data->paymentid = $id;
